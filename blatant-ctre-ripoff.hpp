...
//These (currently unnamed) regexes are similar to ECMAScript regexes, with the following differences:
//- It's C++.
...
//- Mine doesn't use consteval, or any other post-C++14 features.
...
auto mkstr(T get_str, std::index_sequence<Ns...>)
{
...
struct pair {
	const char * start;
	const char * end;
	template<typename T> T as() { return T(start, end); }
};
...
public:
	...
	// careful with passing temporaries; if it's a std::string, that's a use-after-free (string_view is safe)
	template<typename T>
	static match_t<n_capture> match(const T& str)
	{
	...
	template<typename T>
	static match_t<n_capture> search(const T& str)
	{
	...
	static T2 replace(const T& str, const char * replacement)
	{
